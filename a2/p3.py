...
class Scheduler:
    ...
    def manhattan_score(self, move):
        ...
        closest_ghost = (len(self.state) + len(self.state[0]) + 1) * 500
        closest_food = closest_ghost
        for u, v in [(x.i, x.j) for x in self.ghosts]:
            closest_ghost = min(closest_ghost, abs(i-u)+abs(j-v))
        
        for u, v in [list(map(int, f.split(','))) for f in self.foods]:
            closest_food = min(closest_food, abs(i-u)+abs(j-v))
        # lower is better
